/** =====================================================================
 * NextUp · Apps Script backend · v4.1.1a
...
function setQrCacheFolderById(folderId) {
  ...
  DriveApp.getFolderById(folderId); // validate
...
function _parseEventsSheet_(values) {
  ...
  for (var r=1;r<values.length;r++){
    ...
    out.push({
      id: id || ('ev_' + _sha1hex_(name).slice(0,8)),
      slug: slug || id || '',
      name: name || slug || id,
...
function _loadEventsFromSource_() {
  ...
  if (sheetId) {
    try {
      ...
      var events = _parseEventsSheet_(values);
...
function getPublicBundle(eventKey) {
  try {
    ...
    return {
      ...
      eventMeta:{ id:meta.id, name:meta.name, slug:meta.slug||meta.id, dateISO:meta.startDateISO,
                  status: meta.status || 'LINKS_READY', formUrl: meta.formUrl || '' }
    };
  } catch(e) { return { ok:false, error:String(e&&e.message||e) }; }
}
...
/** =====================================================================
 * CONTROL + EVENT FLOW (automated, self-healing)
 * ===================================================================== */
function setControlTemplateId(templateSpreadsheetId) {
  ...
  SpreadsheetApp.openById(templateSpreadsheetId); // validate
...
function ensureControlWorkbook() {
  ...
  if (!tplId) return { ok:false, error:'control_template_not_configured' };
  ...
  try {
    ...
    if (cid) {
      try {
        var ex = SpreadsheetApp.openById(cid);
        var v = _validateWorkbookTabs_(ex, required);
    ...
    for (var i=0;i<3;i++){
      ...
      var v2 = _validateWorkbookTabs_(ctrl, required);
...
function getControlStatus() {
  var p = PropertiesService.getScriptProperties();
  var tpl = p.getProperty(_K.CTRL_TEMPLATE_ID) || '';
  var cid = p.getProperty(_K.CTRL_BOOK_ID) || '';
  var present=false, err='', missingTabs=[];
  if (cid) {
    try {
      var ss = SpreadsheetApp.openById(cid);
      present = true;
      var v = _validateWorkbookTabs_(ss, _getRequiredTabs_());
      if (!v.ok) { present=false; missingTabs=v.missing; }
    } catch(e){ err=String(e&&e.message||e); }
  }
  return { ok:true, templateId: tpl, controlId: cid, present: present, missingTabs: missingTabs, err: err };
}
