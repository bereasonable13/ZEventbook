...
function getControlTemplateSpec_() {
    ...
    return [
        // Core admin/indexing
        { name: 'Events', headers: ['id', 'name', 'slug', 'startDateISO', 'flow', 'seedMode', 'formId', 'eventSpreadsheetUrl', 'eventTag', 'isDefault'] },
...
const DIAG = {
    ...
    log(level, where, msg, data) {
        try {
            ...
            // Last resort: swallows; we never throw from logging.
...
/** Compute the final workbook title (stable + readable). */
function eventWorkbookTitle_(name, slug, dateISO, id) {
    // Keep your prior “slug-date” compatibility, but add human name up front.
    // Example: "NextUp · Fall Open 2025 · 2025-10-12 · fall-open-2025"
...
/** Create Flow (workbook first, then index in Control). */
function createEvent(payload) {
    ...
    if (!name || !dateISO) {
        DIAG.log('error', 'createEvent', 'missing name or date', { payload });
        return { ok: false, phase: 'validate', error: 'Name and Date required' };
    }

    // Compute slug & tag now (consistent with your existing code).
    const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || `event-${Date.now()}`;
    const id = Utilities.getUuid();
    const tag = computeEventTag_(slug, dateISO, id);

    DIAG.log('info', 'createEvent', 'begin', { id, name, dateISO, slug, tag, seedMode, elimType });

    try {
        // 1) Create workbook FIRST (per your new requirement)
        const folderId = cfgEventsFolderId_();
        const title = eventWorkbookTitle_(name, slug, dateISO, id);

        // If a template is configured, copy it; else build from Control templates
        const templateId = cfgTemplateId_();
        let ss, ssId, ssUrl;
        ...
        if (templateId) {
            ...
            // Ensure expected tabs exist & headers (idempotent)
            Object.values(EVENT_TABS).forEach(n => { if (!ss.getSheetByName(n)) ss.insertSheet(n); });
            header_(ss, EVENT_TABS.SIGNUPS, ['timestamp', 'name', 'email', 'phone', 'team', 'notes']);
            header_(ss, EVENT_TABS.SCHEDULE, ['round', 'time', 'activity', 'notes', 'table']);
            header_(ss, EVENT_TABS.STANDINGS, ['team', 'points', 'tiebreak', 'notes']);
            ...
            // Build from Control-provided templates (your Path B)
            ss = createEventsFolderSpreadsheet_(title); // you already have this
            ssId = ss.getId();
            ssUrl = ss.getUrl();
            ...
            // Headers via Control templates (keeps your fallback behavior)
            ...
            // Poster defaults from Control
            const posterKv = ctlPosterDefaults_();
            const poster = ensureKvSheet_(ss, EVENT_TABS.POSTER);
        ...
        // OPTIONAL: Mirror a one-row Events sheet inside the event workbook (for “local” tracking).
        ...
        mirror.getRange(2, 1, 1, mirrorHeaders.length).setValues([[
            id, name, slug, dateISO, ssId, ssUrl, '', tag, false, seedMode, elimType
        ]]);

        DIAG.log('info', 'createEvent', 'workbook.created', { id, ssId, ssUrl, title });

        // 2) Now index it in Control.Events (master tracking)
        const ctl = getEventsSheet_();
        const row = [id, name, slug, dateISO, ssId, ssUrl, '', tag, false, seedMode, elimType, '', '', ''];
        ctl.appendRow(row);
        bustEventsCache_();

        DIAG.log('info', 'createEvent', 'control.indexed', { id, rowCount: ctl.getLastRow() });

        // 3) Return payload optimized for UI “step” toasts
        const ended = new Date();
        return {
            ok: true,
            id, slug, tag, ssId, ssUrl,
            phase: 'done',
            ms: ended.getTime() - started.getTime()
        };
    } catch (e) {
        DIAG.log('error', 'createEvent', 'exception', { err: String(e), stack: e && e.stack });
